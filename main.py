...
parser.add_argument('--targetH', type=int, default=32,
                    help='the width of the input image to network')
parser.add_argument('--targetW', type=int, default=100,
                    help='the width of the input image to network')
...
parser.add_argument('--MORAN', default='',
...
parser.add_argument('--displayInterval', type=int,
                    default=500, help='Interval to be displayed')
...
parser.add_argument('--saveInterval', type=int,
                    default=10000, help='Interval to be displayed')
...
parser.add_argument('--BidirDecoder', action='store_true',
                    help='Whether to use BidirDecoder')
...
train_dataset = dataset.lmdbDataset(rootfile='new_gt.txt', transform=dataset.resizeNormalize(
    (opt.imgW, opt.imgH)), reverse=opt.BidirDecoder, mode=True)
...
test_dataset = dataset.lmdbDataset(rootfile='new_gt.txt',
                                   transform=dataset.resizeNormalize((opt.imgW, opt.imgH)), reverse=opt.BidirDecoder, mode=False)
...
if opt.cuda:
    MORAN = MORAN(nc, nclass, opt.nh, opt.targetH, opt.targetW,
                  BidirDecoder=opt.BidirDecoder, CUDA=opt.cuda)
else:
    MORAN = MORAN(nc, nclass, opt.nh, opt.targetH, opt.targetW,
                  BidirDecoder=opt.BidirDecoder, inputDataType='torch.FloatTensor', CUDA=opt.cuda)
...
# setup optimizer
if opt.adam:
    optimizer = optim.Adam(MORAN.parameters(), lr=opt.lr,
